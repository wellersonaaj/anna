...
const isPublicPath = (pathname) => pathname === "/login" || pathname.startsWith("/fila/");
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                const preservedBrecho = currentBrecho && me.memberships.some((m) => m.brecho.id === currentBrecho.id)
                    ? currentBrecho
                    : me.activeBrecho;
                setSession({
                    accessToken,
