    ...
    return (_jsxs(AppShell, { showTopBar: true, showBottomNav: true, activeTab: "vendas", children: [_jsx(TopShortcutBar, { shortcuts: [
                    ...
                    { id: "entregues", label: "Entregues" }
                                ...
                                return (_jsxs("article", { className: "rounded-3xl border border-rose-50 bg-white p-4 shadow-sm", children: [_jsxs("div", { className: "mb-3 flex items-start justify-between gap-2", children: [_jsxs("div", { children: [_jsx("h3", { className: "font-bold text-gray-900", children: sacola.cliente.nome }), _jsxs("p", { className: "text-sm text-gray-500", children: [sacola.totalPecas, " pe\u00E7a(s) na sacola"] })] }), _jsx(Link, { to: `/clientes/${sacola.cliente.id}`, className: "text-xs font-bold text-primary", children: "Ver cliente" })] }), _jsx("ul", { className: "mb-3 space-y-2", children: sacola.vendas.map((venda) => (_jsxs("li", { className: "flex items-start gap-2 text-sm", children: [_jsx("input", { type: "checkbox", className: "mt-1", checked: selected.includes(venda.id), onChange: (e) => {
                                                        ...
                                                        } }), _jsxs("div", { className: "flex-1", children: [_jsxs("span", { children: [venda.peca.codigo ? `${venda.peca.codigo} · ` : "", venda.peca.nome] }), _jsxs("div", { className: "mt-0.5 flex flex-wrap items-center gap-2", children: [_jsx("span", { className: "text-xs font-semibold text-primary", children: formatCurrency(venda.precoVenda) }), computeLucroBruto(parseMoneyLike(venda.precoVenda), venda.precoCusto != null ? parseMoneyLike(venda.precoCusto) : null) != null && (_jsxs("span", { className: "text-xs font-semibold text-green-700", children: ["Lucro ", formatCurrency(computeLucroBruto(parseMoneyLike(venda.precoVenda), venda.precoCusto != null ? parseMoneyLike(venda.precoCusto) : null))] })), _jsx("span", { className: `rounded px-1.5 py-0.5 text-[10px] font-bold ${venda.freteIncluso ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-600"}`, children: formatFreteInclusoLabel(venda.freteIncluso, venda.freteInclusoValor) }), _jsx("button", { type: "button", className: "text-[10px] font-bold uppercase text-primary underline", onClick: () => setEditingSale({
                                                                            id: venda.id,
                                ...
                                const lucroBruto = computeLucroBruto(parseMoneyLike(sale.precoVenda), sale.precoCusto != null ? parseMoneyLike(sale.precoCusto) : null);
                                return (_jsx("article", { className: "rounded-2xl border border-rose-50 bg-white p-3 shadow-sm", children: _jsxs("div", { className: "flex items-center gap-4", children: [pecaImg ? (_jsx("img", { src: pecaImg, alt: `Foto da peça ${sale.peca.nome}`, className: "h-14 w-14 rounded-xl object-cover", loading: "lazy" })) : (_jsx("div", { className: "flex h-14 w-14 items-center justify-center rounded-xl bg-surface-container-low text-xs text-outline", children: "Sem foto" })), _jsxs("div", { className: "flex-1", children: [_jsx("h3", { className: "font-bold text-gray-900", children: sale.peca.nome }), _jsxs("p", { className: "text-xs text-gray-500", children: [sale.cliente.nome, " \u2022", " ", new Date(sale.entrega?.entregueEm ?? sale.criadoEm).toLocaleDateString("pt-BR")] }), lucroBruto != null && (_jsxs("p", { className: "text-xs font-semibold text-green-700", children: ["Lucro ", formatCurrency(lucroBruto)] }))] }), _jsx("strong", { className: "text-primary", children: formatCurrency(sale.ganhosTotal) })] }) }, sale.id));
