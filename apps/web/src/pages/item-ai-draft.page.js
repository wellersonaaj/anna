...
import { addItemFoto, analisarFotoRascunho, createFotoLote, createItem, listAcervoSuggestions, presignFotoLoteUpload, putToPresignedUrl } from "../api/items";
...
export const ItemAIDraftPage = () => {
    ...
    const { imageDataUrl, textoContexto, analysis, formValues, setImageDataUrl, setTextoContexto, setFormField, applyAnalysis, resetDraft } = useItemAIDraftStore();
    ...
    return (_jsxs(AppShell, { children: [_jsx(Link, { to: "/", children: "\u2190 Voltar ao estoque" }), _jsxs("header", { children: [_jsx("h1", { style: { marginBottom: 4 }, children: "Cadastrar com IA" }), _jsx("p", { style: { marginTop: 0, opacity: 0.85 }, children: "Envie uma foto, opcionalmente descreva o contexto, revise os campos e conclua." })] }), actionError && (_jsx("p", { style: { color: "#b60e3d", fontSize: 14 }, role: "alert", children: actionError })), _jsx(Section, { title: "1. Foto e contexto", children: _jsxs("div", { className: "stack", style: { gap: 10 }, children: [_jsxs("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" }, children: [_jsx(Button, { type: "button", onClick: () => cameraInputRef.current?.click(), children: "Tirar foto" }), _jsx(Button, { type: "button", onClick: () => galleryInputRef.current?.click(), children: "Escolher da galeria" })] }), _jsx("input", { ref: cameraInputRef, type: "file", accept: "image/*", capture: "environment", hidden: true, onChange: (event) => {
                                void handleImagePicked(event.target.files?.[0] ?? null);
                                event.target.value = "";
                            } }), _jsx("input", { ref: galleryInputRef, type: "file", accept: "image/*", hidden: true, onChange: (event) => {
                                void handleImagePicked(event.target.files?.[0] ?? null);
                                event.target.value = "";
                            } }), imageDataUrl ? (_jsx("img", { src: imageDataUrl, alt: "Pr\u00E9-visualiza\u00E7\u00E3o da foto selecionada", style: { width: "100%", maxWidth: 360, borderRadius: 12, border: "1px solid #e7d5d6" } })) : (_jsx("p", { style: { opacity: 0.8, margin: 0 }, children: "Nenhuma foto selecionada ainda." })), _jsx(Field, { label: "Contexto em texto (opcional)", children: _jsx("textarea", { value: textoContexto, onChange: (event) => setTextoContexto(event.target.value), rows: 3, style: {
                                    width: "100%",
                                    ...
                                    fontFamily: "inherit",
                                    fontSize: 15
                                }, placeholder: "Ex.: vestido com brilho, costas com z\u00EDper e caimento reto..." }) }), _jsx(Button, { type: "button", onClick: () => analyzeMutation.mutate(), disabled: !imageDataUrl || analyzeMutation.isPending, children: analyzeMutation.isPending ? "Analisando..." : "Sugerir com IA" })] }) }), _jsxs(Section, { title: "2. Revis\u00E3o r\u00E1pida dos dados", children: [analysis && (_jsxs("div", { style: {
                            fontSize: 13,
                            padding: 12,
                            background: "#f0f7f2",
                            borderRadius: 10,
